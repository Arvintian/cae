<template>
  <div>
    <Card>
      <Tables ref="tables" searchable border search-place="top" v-model="routerList" :columns="columns" :pageSize="pageSize">
    ...
    <Modal v-if="editForm" v-model="editModalVisible" :title="editForm.action === 'add' ? '创建路由' : '更新路由'" :mask-closable="false" @on-cancel="closeEditFormModal()" witdth="560">
      <Form ref="editForm" :model="editForm" :rules="editFormRule" :label-width="100">
        ...
        <Form-item label="HTTPS域名：" prop="ssl_domain" :rules="editFormRule.ssl_domain">
          <Input v-model.trim="editForm.ssl_domain"></Input>
        </Form-item>
        <Form-item label="HTTPS跳转：" prop="ssl_redirect" :rules="editFormRule.ssl_redirect">
          <i-switch v-model="editForm.ssl_redirect" />
        </Form-item>
      </Form>
      <div slot="footer">
      ...
      this.$refs["editForm"].validate((valid) => {
        if (valid) {
          ...
          let payload = {
            ...
            ssl_redirect: data.ssl_redirect
          };
          applyRouter(payload)
            .then((res) => {
              let data = res.data;
